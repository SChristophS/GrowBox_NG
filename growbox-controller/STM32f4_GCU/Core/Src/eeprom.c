...
bool EEPROM_Erase(void) {
    ...
    uint8_t emptyData[64];  // Kleiner Puffer
    HAL_StatusTypeDef status;

    // Fülle den Puffer mit 0xFF
    memset(emptyData, 0xFF, sizeof(emptyData));

    // EEPROM-Größe in Bytes
    uint32_t totalSize = EEPROM_SIZE; // Annahme: EEPROM_SIZE ist 32768

    while (memAddress < totalSize) {
        // Schreibe die Daten
        status = HAL_I2C_Mem_Write(&hi2c2, EEPROM_DEVICE_ADDRESS, memAddress, I2C_MEMADD_SIZE_16BIT, emptyData, pageSize, 1000);
        if (status != HAL_OK) {
            printf("EEPROM_Erase: Error writing at address 0x%04X\r\n", memAddress);
            return false;
        }
        ...
        printf("EEPROM_Erase: Writing %d bytes to address 0x%04X\r\n", pageSize, memAddress);
...
bool EEPROM_Write(uint16_t memAddress, uint8_t *data, uint16_t size) {
    ...
    while (bytesWritten < size) {
        ...
        // Korrekte Ausgabe der geschriebenen Bytes
        printf("EEPROM_Write: Writing %d bytes to address 0x%04X\r\n", bytesToWrite, memAddress);
